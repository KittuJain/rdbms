conn / as sysdba
-- @D:/rdbms/test_sql/test_schema_01.sql;
...
SELECT E.ID, E.NAME, D.DEPT_NAME FROM EMPLOYEE E NATURAL JOIN DEPARTMENT D ON (E.DEPT_ID = D.DEPT_ID);
...
-- ALL DEPT_ID WHICH DON'T HAVE ANY EMPLOYEES
SELECT ID FROM DEPARTMENT 
MINUS SELECT DEPT_ID FROM EMPLOYEE;
...
-- GET HIGHEST SALARY IN PRODUCTION DEPT
SELECT MAX(SALARY) FROM EMPLOYEE E JOIN DEPARTMENT D ON(E.DEPT_ID = D.ID) WHERE D.DEPT_NAME = 'PRODUCTION';
...
SELECT pan_number FROM EMPLOYEE GROUP BY pan_number;
...
-- EMPLOYEES WHICH HAVE HIGHEST SALARY WITH EMPLOYEE NAME

SELECT E.NAME, D.DEPT_NAME,COUNT(*), MAX(E.SALARY) AS HIGHEST_SALARY
	FROM DEPARTMENT D JOIN EMPLOYEE E 
	ON(E.DEPT_ID = D.ID) 
	GROUP BY (E.NAME, D.DEPT_NAME);

-- get HIGHEST_SALARY for each gender in a dept

SELECT E.GENDER, D.DEPT_NAME,COUNT(*), MAX(E.SALARY) AS HIGHEST_SALARY
	FROM DEPARTMENT D JOIN EMPLOYEE E 
	ON(E.DEPT_ID = D.ID) 
	GROUP BY (E.GENDER, D.DEPT_NAME);

SELECT D.DEPT_NAME, E.GENDER,COUNT(*), MAX(E.SALARY) AS HIGHEST_SALARY
	FROM DEPARTMENT D JOIN EMPLOYEE E 
	ON(E.DEPT_ID = D.ID) 
...
-- EMPLOYEES WHOSE MINIMUM SALARY IS LESS THAN 10,000
SELECT D.DEPT_NAME, E.ID, E.NAME, E.SALARY
	FROM EMPLOYEE E JOIN DEPARTMENT D 
	ON (E.DEPT_ID=D.ID)
	GROUP BY(D.DEPT_NAME, E.ID, E.NAME, E.SALARY) HAVING E.SALARY<10000;

-- employees ID, NAME OF EMPLOYEES IN EACH DEPT IS LESS THAN THE AVG SALARY IN THE DEPT_ID

-- SELECT D.DEPT_NAME, E.ID, E.NAME
-- 	FROM DEPARTMENT D JOIN EMPLOYEE E 
-- 	ON (E.DEPT_ID=D.ID)
-- 	GROUP BY(D.DEPT_NAME, E.ID, E.NAME, E.SALARY) HAVING E.SALARY<AVG(E.SALARY);
...
---------- MINIMUM SALARY OF EACH DEPT SHOULD CONTAIN ATLEAST TWO EMPLOYEES ----------

-- SELECT D.DEPT_NAME

---------- SUB-QUERIES -------- NAMED AS INLINE VIEW
SELECT * FROM (SELECT * FROM EMPLOYEE);
SELECT * FROM (SELECT ID FROM EMPLOYEE);
-- SELECT * FROM (SELECT * FROM EMPLOYEE)EMP DEPARTMENT D WHERE D.ID=EMP.DEPT_ID;

-- JOIN OPERATION MODIFIED USING SUB-QUERIES
SELECT DEPT_NAME
	FROM DEPARTMENT
	WHERE ID
	IN (SELECT DEPT_ID FROM EMPLOYEE);

-------- ANTI - JOIN GET THINGS THAT DOESN'T SATISFIES JOIN CONDITION
SELECT DEPT_NAME 
	FROM DEPARTMENT 
	WHERE ID 
	NOT IN (SELECT DEPT_ID FROM EMPLOYEE WHERE DEPT_ID IS NOT NULL);
	
-- SAME QUERY USING NVL FUNCTION
SELECT DEPT_NAME 
	FROM DEPARTMENT 
	WHERE ID 
	NOT IN (SELECT NVL(DEPT_ID, 0) FROM EMPLOYEE);

-- emp_id, emp_name, salary, dept_name, avg(salary) of dept_id

SELECT DEPT_ID, AVG(SALARY) FROM EMPLOYEE GROUP BY(DEPT_ID);


SELECT E.ID, E.NAME, E.DEPT_ID,E.SALARY, DEPT_SALARIES.AVG_SALARY FROM EMPLOYEE E ,
(SELECT E.DEPT_ID , AVG(E.SALARY) AS AVG_SALARY
FROM EMPLOYEE E 
WHERE E.DEPT_ID IS NOT NULL
GROUP BY E.DEPT_ID) DEPT_SALARIES
WHERE E.DEPT_ID = DEPT_SALARIES.DEPT_ID;

-- same using OVER PARTITION BY for this query
SELECT ID, NAME, DEPT_ID, ROUND(AVG(SALARY) OVER (PARTITION BY DEPT_ID)) FROM EMPLOYEE;